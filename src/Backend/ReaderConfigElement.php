        ...
        switch (Input::get('act')) {
            ...
            case 'create':
                if (!strlen(Input::get('pid')) || !in_array(Input::get('pid'), $root, true)) {
                    throw new AccessDeniedException(
                        'Not enough permissions to create reader_config_element items in reader_config_element archive ID '.Input::get('pid').'.'
                    );
                }
                break;
            ...
            case 'copy':
                if (!in_array(Input::get('pid'), $root, true)) {
                    throw new AccessDeniedException(
                        'Not enough permissions to '.Input::get('act').' reader_config_element item ID '.$id
            ...
            case 'copyAll':
                if (!in_array($id, $root, true)) {
                    throw new AccessDeniedException(
                        'Not enough permissions to access reader_config_element archive ID '.$id.'.'
                    );
                }
